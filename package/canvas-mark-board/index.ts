...
export default class CanvasMarkBoard implements ICanvasMarkBoard {
  ...
  appMousedown(e: MouseEvent): void {
    ...
    e.buttons === 1 && this.emit("onmousedown", point);
  }
  /** 鼠标抬起事件 */
  ...
  public addObjectData() {
    ...
    if (this.currentDrawingType) {
      try {
        obj = new markMap[this.currentDrawingType](this);
      } catch (err) {
        throw new Error(
  ...
  public setObjectData(list: IMarkObjectJSON[]) {
    list.forEach((item) => {
      ...
      if (item.type) {
        try {
          obj = markMap[item.type].import(this, item);
        } catch (err) {
  ...
  /** 设置对象标签 */
  public setObjectLabel(id: IMarkObjectId, data: IObjectLabelData) {
  ...
  /** 设置移动状态 */
  public setMoveStatus(status: boolean) {
